...
- `id="contact"` の `<section>`・`"use client"` コンポーネント
- フォーム項目: お名前・メールアドレス・相談内容（ドロップダウン）・詳細
- ドロップダウン選択肢: 業務自動化（VBA / Python）/ データ連携・一元管理 / 現場目線の改善提案 / フロントエンド開発・Webサイト制作 / その他
- 送信ボタンを押すと「送信されました。」メッセージに切り替わる（バックエンド連携は未実装）
